(() => {
  ...
  const steps=[
    ...
    {id:"preview",type:"result",title:"Com base nas suas respostas:",body:"Veja uma estimativa de planejamento para a sua meta.",testimonials:true,testimonialStart:1,personalizedTitle:true},
    ...
    {id:"motivation",type:"result",title:"Com constância, sua meta fica mais fácil de acompanhar passo a passo.",body:"Veja a estimativa de planejamento para a sua meta.",testimonials:true,testimonialStart:5},
  ...
  let stepLoadingId,stepLoadingAdvanceId,selectionAdvanceId;
  ...
  function showSelectionConfirmation(label,callback){
    clearTimeout(selectionAdvanceId);
    const main=document.querySelector(".quiz-page");
    main?.classList.add("selection-locked");
    document.querySelector(".selection-confirmation")?.remove();
    const box=document.createElement("div");
    box.className="selection-confirmation";
    box.setAttribute("role","status");
    box.innerHTML=`<span>✓</span><p>${esc(label)}<br><small>Resposta registrada</small></p>`;
    document.body.appendChild(box);
    selectionAdvanceId=setTimeout(()=>{box.remove();main?.classList.remove("selection-locked");callback()},460);
  }

  function renderStep(animate=true){
    animate=animate&&activeTrackedStep!==state.current;
    clearInterval(stepLoadingId);clearTimeout(stepLoadingAdvanceId);clearTimeout(selectionAdvanceId);
    document.querySelector(".selection-confirmation")?.remove();
  ...
  function bind(s){
    ...
    document.querySelectorAll("[data-option]").forEach(b=>b.addEventListener("click",()=>{
      ...
      showSelectionConfirmation(picked,next);
    }));
  ...
  function personalizedPriorities(){
    ...
    const goal=target?{pain:`Sua meta registrada: ${target} kg`,solution:`Você respondeu que quer chegar aos ${target} kg${current?` partindo de ${current} kg`:""}. ${timeline.label.charAt(0).toUpperCase()+timeline.label.slice(1)} é a referência de planejamento mostrada no quiz; o primeiro ciclo continua sendo de 4 semanas e a evolução deve ser reavaliada ao longo do processo.`}:{pain:"Sua meta de peso",solution:"Seu plano começa com um ciclo de 4 semanas e organiza ações práticas para acompanhar sua evolução em direção ao peso desejado."};
